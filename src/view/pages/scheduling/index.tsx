...
} from "../../../@shared/contexts/Patients/PatientContext";
...
} from "../../../@shared/contexts/Scheduling/SchedulingContext";
...
function Page() {
  const [selectedScheduling, setSelectedScheduling] = useState<string | null>(
    null
  );
  ...
  const selectedSchedulingData = selectedScheduling
    ? schedulingsData.find(
        (scheduling: { id: string }) => scheduling.id === selectedScheduling
      )
    : null;
  ...
  return (
    <div className="rounded-md bg-white">
      <div className="flex flex-col md:flex-row items-start md:items-end justify-between pt-6 px-6 lg:px-8 ">
        ...
        <Button onClick={() => setOpenCreateSchedulingModal(true)}>
          ...
          columns={[
            {
              ...
              key: "dataAgendamento",
              render: (rowData) =>
                format(new Date(rowData.dataAgendamento), "dd/MM/yyyy"),
            },
            {
              header: "Empresa",
              key: "enterprise",
            ...
            },
            {
              ...
              key: "parecer",
              render: (rowData) =>
                rowData.parecer == null ? "---" : rowData.parecer,
            },
      ...
      <Modal
        open={openCreateSchedulingModal}
        onClose={() => setOpenCreateSchedulingModal(false)}
      >
        <CreateSchedulingForm />
      </Modal>

      <Modal
        open={openUpdateSchedulingModal}
        onClose={() => setOpenUpdateSchedulingModal(false)}
      >
        {selectedSchedulingData && (
          <UpdateSchedulingForm scheduling={selectedSchedulingData} />
        )}
      </Modal>
      ...
      >
        <div className="flex flex-col gap-4 mt-5 w-[50vw] max-h-[80vh] overflow-y-auto p-4 border rounded-md">
          <div className="flex gap-4">
            <Input
              label="CPF"
              disabled
              value={selectedSchedulingData?.patient?.cpf}
            />
            <Input
              label="Nome"
              disabled
              value={selectedSchedulingData?.patient?.name}
            />
          </div>
          <Input
            label="Data de Nascimento"
            disabled
            value={selectedSchedulingData?.patient?.dateBirthday}
          />
          <Input
            label="Email"
            disabled
            value={selectedSchedulingData?.patient?.email}
          />
          <div className="flex gap-4">
            <Input
              label="CEP"
              disabled
              value={selectedSchedulingData?.patient?.cep}
            />
            <Input
              label="Rua"
              disabled
              value={selectedSchedulingData?.patient?.street}
            />
          </div>
          <div className="flex gap-4">
            <Input
              label="Número"
              disabled
              value={selectedSchedulingData?.patient?.number}
            />
            <Input
              label="Complemento"
              disabled
              value={selectedSchedulingData?.patient?.complement}
            />
          </div>
          <Input
            label="Bairro"
            disabled
            value={selectedSchedulingData?.patient?.neighborhood}
          />
          <Input
            label="Cidade"
            disabled
            value={selectedSchedulingData?.patient?.city}
          />
          <Input
            label="Estado"
            disabled
          ...
          <div>
            {selectedSchedulingData?.performedExams?.length ? (
              <ul className="space-y-4">
                {selectedSchedulingData.performedExams.map(
                  (
                    exam: {
                      specialty: any;
                      category: any;
                      dataRealizacaoExameLaboratorial: string | number | Date;
                      dataResultadoExameLaboratorial: string | number | Date;
                      laboratoryResultUrl: string | undefined;
                    },
                      ...
                      <div className="flex flex-col mt-2">
                        ...
                        <span className="text-gray-900">
                          {exam.dataRealizacaoExameLaboratorial
                            ? format(
                                new Date(exam.dataRealizacaoExameLaboratorial),
                                "dd/MM/yyyy"
                              )
                            : "---"}
                        </span>
                      ...
                      <div className="flex flex-col mt-2">
                        ...
                        <span className="text-gray-900">
                          {exam.dataResultadoExameLaboratorial
                            ? format(
                                new Date(exam.dataResultadoExameLaboratorial),
                                "dd/MM/yyyy"
                              )
                            : "---"}
                        </span>
